...
### 9.1 Stage-1 addendum: implementation pins (2026-07-05, disclosed for the §9 reviewer pass)

Implementing §9 (checklist item 2; `crates/pen-eval/src/runtime_field.rs`) required fixing four sub-choices. They are recorded here so the §9 reviewer pass judges them alongside §9 itself; none were tuned against any output (no crossing has been computed; the loop cannot compute one).

1. **Field refresh at step boundaries (claimed forced, P-window).** The bindable domain of cadence step k is the set of entries sealed *before* step k; entries sealed within step k become bindable at step k+1. R1 dedup, by contrast, always runs against the full accepted history, including within-step seals (P-ν is not windowed). The alternative — refreshing the bindable domain within the step — makes the first step run to the permanent exhaustion of the entire reachable closure, leaving every later step empty by construction and contradicting §2's per-step series; eliminated by the same reductio that eliminated R-F3(i).
2. **Bar convention (engine precedent, fresh-stratum ledger).** The n-th acceptance of the stratum faces Bar_n = Φ_n·Ω_{n−1} on the stratum's own ledger (plain cumulative Ω = Σν/Σκ over prior acceptances; Bar_1 = 0 on the empty ledger — the arena precedent). Clearing is ρ ≥ Bar_n; "smallest sufficient overshoot" is minimal ρ − Bar_n among clearing candidates, ties broken by the deterministic enumeration order (presentation convention, gauge).
3. **σ generalized to sealing order (claimed forced by §1's definition).** With tuple bindings there is no single "level" per entry; σ maps each stratum entry to its successor in the stratum's sealing order. The R1 discount in field form: a clause scores 0 iff its canonical presentation is already accepted, or its σ-preimage's is. Restricted to single-chain instantiation this is exactly the Stage-1 rule of §8 (verified by unit test).
4. **Bounded search (disclosed policy, NOT claimed forced).** Candidates are partial-binding vectors: every slot (stratum-internal Var occurrence) either keeps its frozen internal form or binds a sealed entry — excluding partial bindings would be an arity cap by another name. Enumeration is deterministic and lexicographic, per slot keep-internal first then the domain newest-first (adjacency priority, P-window); the all-internal vector, the frozen schema itself, is candidate one. Scoring stops at a per-round budget (`FieldPolicy::search_budget`). Budget and order are lane policy in the Genesis-relaxed-lane sense; the mechanics artifact (`examples/runtime_mechanics.rs` → ν profiles only) carries the budget sensitivity check that must accompany the §9 freeze. **Dissolution path:** the per-round field is finite ((domain+1)^slots vectors), so a budget at or above the round size makes the enumeration *exhaustive* — the budget then ceases to be policy and the enumeration order reduces to a tie-break among equal-overshoot candidates (a presentation convention, gauge). For the frozen v2 variants the rounds are small enough to run exhaustively (`--budgets exhaustive`); the freeze artifact should be produced in that mode, retiring the budget dial for these strata. The budget survives only as a resource guard for larger schemas, where its disclosure duty returns.

Admissibility inside the loop is ρ > 0 (the v1 lesson) and nothing else: dedup and the bar do the limiting, per this section's own discipline. The pen-type structural-family packages are *not* applied to stratum instantiations — no pinned mapping from Genesis families to stratum instantiations exists, and improvising one would be a dial. Flagged for the reviewer. Constructive primeness is enforced by construction (every candidate is a single schema instantiation, never a package). The Stage-1 exclusion is unchanged: the loop never consults Δ_k or any engagement fraction, and no crossing index is computable from the addendum's code.

**Reviewer finding, applied (2026-07-05, post-artifact).** The first mechanics artifact run (single-chain profiles [7,0,…], [10,15,0,…], [12,24,0,…] — confirming the §8 corollary exactly — and field profiles of exactly one seal, the frozen schema, at every budget) exposed a dial in the addendum's first version: a structural connectivity gate (`is_connected`) sat inside the loop's admissibility. Binding any slot to a sealed entry severs the v2 schemas' de Bruijn chains (the binding-path PathCon clause is orphaned), so the gate excluded the *entire* tuple field — including the σ-chain continuations that RC-1's own path scores, an internal inconsistency between the §8 and §9 code paths. The gate contradicted this section's "the field is generated by the grammar; dedup and the bar do the limiting" and had no P-admissibility pin (§3's pin concerns depth-1 obligations, not graph shape). Resolution, adopted with the schedule's owner: the connectivity gate is removed and partial bindings are included (both recorded above). The superseded artifact is retained in history; the artifact must be regenerated under the corrected field before the §9 freeze. No engagement fraction or crossing was computed in reaching this finding.
