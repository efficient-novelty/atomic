...
QuotientActionIntroducedLineageRowPlanSharedV1 =
  (lineage_ordinal:u32,
   ...
   path_bindings:QuotientSupportActionLoweredPathBindingsPlanSharedV1,
...
admitted. The remaining three fields are exactly the census companion fields
of subsection 10.18.2 in their lowered shared form.
...
Checked row admission requires:

```text
row.path_bindings.dependency.root = ActionArguments
row.result_schema                = Term
row.path_bindings.dependency.result_schema = row.result_schema
row.lineage_ordinal              = the row's physical position.
```

The `Term` requirement is exact: subsection 10.18.2 fixes that every paired
native descendant locus has nominal schema `Term`, and the companion copies
its paired row's `nominal_result_schema` byte-for-byte. No companion-binding
constant beyond these is asserted at this cut; the row retains the complete
lowered path-binding value of its census event, whatever optional
grammar/operation-record companions that event carries.
...
CompileActionIntroducedLineage(B,A):
  ...
  require I_A = the one common paired-event census consumed by this
    transaction's step-4 graph construction, by complete ordered bytes
  require L_A = map(LowerActionPassA,I_A) by complete ordered bytes

  rows = []
  for checked i in [0,L_A.len):
    if I_A[i].edge=ActionIntroduced=27:
      require I_A[i] is the exact IntroCompanion(p,r) of its earlier paired
        native row r at the same local-vector index, with every non-edge
        field byte-identical to r
      rows.push(
        (lineage_ordinal=checked_u32(rows.len),
         primitive=FormalId(p),
         path_bindings=L_A[i].path_bindings,
         result_schema=L_A[i].result_schema,
         expected_judgment=L_A[i].expected_judgment))
...
re-emitted by the frozen S1c2 pass-A compiler from `(B,A,S_A)` and lowered
through the frozen nineteen-node `M_shared`; lineage projection and graph
construction consume one common census within one transaction. A stored
lineage, graph, or eight-group value is never traversed to generate an
event. `FormalId(p)` applies the frozen `EmbedActionPrimitiveLocus`, which
...
unchanged. The three packaged descriptors are typed proposition syntax with
no inhabitant, proof value, or theorem authority. The renaming subject is
...
OPEN-AT-S1C4: the final encoded name, field encoding, and `0xfd` byte
position of the group-6 value — including whether the eventual frozen record
retires the S1a placeholder name `QuotientSemanticSupportCandidateV1` in
favor of this shared plan shape — are determined by ledger item 5 together
with R2e's numerical `TypeIdV1` assignment. This subsection fixes the exact
three-field composition and its occupancy; it does not encode root `0xfd`.
...
1. validate the immutable inputs byte-for-byte: candidate `M_nf`, `G0` and
   ...
   all QG2 bytes;
2. rerun the paired native/edge-27 event census independently
   (`I_A`, `L_A`) and require byte-identity with the frozen S1c2/S1c3 values;
3. project exactly the edge-27 events into the group-4
   action-introduced-lineage value (zero rows for the candidate/identity);
   no later step reads this value;
4. construct the complete shared graph `G_A` by the frozen subsection-10.19.4
