...
## 2. Existing evidence and evidential status

### 2.1 Frozen baseline A0

The existing frozen `gsc-inductive-completion-core-v1` experiment is retained unchanged as baseline evidence:

- the owner-specific `G-Use`/`G-Compute` debt was compiled from H3;
- one quotient class, the direct unit eliminator with its computation equation, was exhaustively found;
- the response was sealed as Act 4; and
- the H4 census was complete and debt-free, so the profile halted.

This result is not rerun or rewritten.
...
Each profile identity is the tuple

```text
(
  bootstrap_digest,
  shared_fragment_digest,
...
No profile may differ from another in an unrecorded component.
...
- trusted kernel and protocol identity;
- production wire and safe-Agda correspondence;
- finite GF2 completeness boundary;
...
## 5. Constitutive hypotheses

### 5.1 C0 — Local inductive completion

`C0` retains the frozen generic reading:

> A public inductive former and its introductions owe their public use/elimination interface and constructor computation interface.

The direct unit eliminator is one possible result of this generic rule, not part of its manifest.

### 5.2 C1 — Contextual internalization

`C1` formalizes Constitutive Self-Containment:

> No operation essential to the formation, substitution, reindexing, or comparison of public possibilities may remain permanently external once it acts nontrivially on sealed public structure.

The compiled obligation is stated only through a finite universal-property interface for the internalization of reindexing along an active sealed extension. The compiler must not name or encode expected mathematical realizations.

Before freeze, `C1` must decide and document:

- the exact target-neutral trigger condition;
...
If the relation between local and contextual ledgers is underdetermined by the registered principles, the alternatives must be separate pre-frozen constitutive profiles. They may not be chosen after observing H3.

## 6. Selective hypotheses
...
and TotalDischarge(H, O_H, x)
and DemandConnected(H, O_H, x)
and FreeSealingAvailable(H, x)
```
...
Unknown
  a proof, complete census, audit value, or resource-dependent decision is unavailable;
...
1. **No sealed extension:** no contextual debt is generated.
2. **Trivial reindexing:** identity or certified-equivalent-to-identity reindexing has the exact predeclared disposition.
3. **Generic non-Genesis extension:** an arbitrary sealed dependent extension compiles the declared universal-interface obligation.
4. **Presentation invariance:** equivalent telescope presentations compile equivalent demand classes.
5. **Renaming invariance:** opaque identifier permutations transport the full census and quotient.
6. **Multiple independent extensions:** support and origin keep their ledgers distinct and composition is explicit.
7. **Window expiry:** an obligation is never silently lost; it is either live, weakened as derived, or retained under an explicit long-lived ledger rule.
8. **Local versus contextual composition:** owner-specific use debt and contextual debt follow the frozen ledger-composition rule.
...
## 9. Pre-registration and freeze protocol
...
- primary endpoints;
- interpretation rules in Section 12; and
- the held-out decoder boundary.
...
Every profile receives the same declared resource envelope. Resource exhaustion is recorded per branch as `Unknown` and is never converted into halt, blockage, uniqueness, or absence.

### 10.5 Delayed comparison

Profile outputs remain sealed and mutually unavailable until all scheduled profiles have terminated or returned `Unknown`. The cross-profile comparison and external mathematical decoder run only afterward.
...
```text
docs/experiments/law-profile-factorial-v1/
  preregistration.md
  ...
  held_out_decoder_report.md
  falsifier_summary.md
```

The comparison report must be generated from sealed artifacts. It must not become an input to any profile.

## 12. Pre-registered interpretation rules
...
### 13.1 Primary target-neutral endpoints

For each profile and branch:

- first guarded live-obligation quotient;
- number of complete discharger classes;
...
“Interesting,” “physical,” or “close to the historical sequence” is not a production endpoint.

### 13.2 Held-out secondary endpoints

Only after sealing:

- external identification with familiar mathematical structures;
- comparison with the historical Genesis corpus;
- length and order similarity;
- later physics-facing interpretation; and
- genuinely new predictions not used in profile formulation.

Held-out agreement can support a profile abductively. It cannot retroactively change generation, selection, or the metric.
...
### Phase K1 — Selective profile freeze
...
Rename or reinterpret the current Phase K as follow-on research after the factorial result. Only profiles that survive their pre-registered falsifiers and interpretation rules may be candidates for longer autonomous runs.
...
## 16. Integration edits to the existing plan
