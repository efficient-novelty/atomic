...
At each step, the selector reads only the current library, the candidate specification, and the immediate inferential surface unlocked by sealing that candidate against the active interface; it does not reward candidates for matching a predeclared future target or a remotely simulated trajectory.
...
\emph{Conjecture:} with sufficiently large compute and materially weaker pruning, the same PEN objective over the same disclosed canonical MBTT space would recover the same 15-step structural trajectory, including the same temporal-cohesive endpoint later semantically read as DCT.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Synthesis} (step 15): In the disclosed canonical MBTT lane, a temporal-cohesive shell, later semantically completed as the Dynamical Cohesive Topos (DCT), fuses spatial logic with Nakano temporal modalities and explicit exchange axioms, clears the selection bar with the largest absolute overshoot in the sequence ($\rho = 12.88$, bar $= 7.40$), and triggers PEN's local halting condition.
\end{enumerate}
...
\begin{itemize}[nosep]
    ...
    \item The \textbf{Terminal Fixed-Point Analysis} shows that, in the current bounded live atomic canonical MBTT lane, the temporal-cohesive endpoint, whose semantic completion is read as DCT, is selected with a large deterministic overshoot and realizes the unique high-interaction structural fixed point available inside the connected component explored by the engine.
\end{itemize}
...
Because the $d=2$ survival bar is later shown to grow with the Fibonacci debt sequence (\cref{cor:fibonacci}), this $O(1)$ grammar dependence can affect only finitely many low-complexity edge cases and is asymptotically negligible. But that asymptotic observation is not empirical protection for the concrete small-$\kappa$ late endpoint: at the current 15-step scale an $O(1)$ grammar shift can still change the ranking, and the current hostile-grammar ablation report keeps exact replay only through Hilbert (step~14) rather than recovering a grammar-independent step-15 temporal shell.
\end{remark}
...
Candidate $X$ is admissible iff it is a well-typed extension over the current library~$\mathcal{B}$, is not definitionally derivable from~$\mathcal{B}$, and satisfies the active horizon bound $\kappa(X) \leq H$.
For bundled specifications, every charged clause of $X$ must be constructively irreducible relative to~$\mathcal{B}$ in the sense of \cref{def:derivability}.
\end{axiom}

\begin{remark}[Admissibility constraints for axiomatic extensions]
\label{rem:admissibility-constraints}
For steps in the axiomatic regime (Steps~10--14), where candidates are type-theoretic axioms rather than explicit constructions, the generic admissibility axiom specializes to four structural constraints:
\begin{enumerate}[nosep]
...
\begin{equation}
    \mathcal{J}_n(X) = \mathcal{P}_{\mathrm{adm}}(X) + \mathcal{P}_{\mathrm{bar}}(X) + \mathcal{P}_{\mathrm{prime}}(X) + \gamma \big( \rho(X) - \mathrm{Bar}_n \big) + \epsilon \kappa(X)
\end{equation}
Where:
\begin{itemize}[nosep]
    \item $\mathcal{P}_{\mathrm{adm}}(X)$ applies an infinite penalty to ill-typed, non-canonical, or definitionally redundant telescopes (enforcing absolute constraint satisfaction).
    \item $\mathcal{P}_{\mathrm{bar}}(X)$ applies an infinite penalty if $\rho(X) < \mathrm{Bar}_n$ (the sub-criticality barrier).
    \item $\mathcal{P}_{\mathrm{prime}}(X)$ applies an infinite penalty when $X$ is constructively composite in the sense of \cref{def:primality}, i.e.\ when a proper terminal SCC sub-bundle already clears the active bar.
...
\end{itemize}
The selected candidate $X_{n+1}$ is the global minimum $\arg\min_X \mathcal{J}_n(X)$. This uniquely and analytically enforces the \emph{minimal positive overshoot} criterion on the admissible, bar-clearing, constructively prime frontier.
\end{axiom}
...
\begin{remark}[Stage-locality and sufficiency]
\label{rem:selection-locality}
The bar $\mathrm{Bar}_n$ should be read as a stagewise sufficiency threshold: PEN does not maximize novelty over all admissible futures, but selects on the current admissible field the least-surplus candidate that is admissible, bar-clearing, and constructively prime. All quantities entering $\mathcal{J}_n(X)$ are computed from the current library state~$\mathcal{B}_n$, the specification of~$X$, and the immediate interaction surface exposed by sealing~$X$ against the active interface. In particular, $\nu(X)$ measures marginal derivational capacity available at the present stage, not reward for a downstream realized trajectory. The selector therefore compares candidates contemporaneously within the current admissible field rather than by forward simulation of remote future libraries.
\end{remark}
...
\[
  \mathrm{Bar}_{16} = \Phi_{16} \cdot \Omega_{15}
  = \frac{F_{16}}{F_{15}} \cdot \frac{\sum_{i \le 15}\nu_i}{\sum_{i \le 15}\kappa_i}
  = \frac{987}{610} \cdot \frac{359}{64} \approx 1.618 \times 5.61 \approx 9.08 .
\]
...
Under fixed fully coupled settings and the disclosed canonical MBTT vocabulary, the PEN objective realizes the temporal-cohesive shell as the terminal high-yield extension, and the current bounded live atomic engine recovers that shell under bounded search.
...
\label{thm:spectral}
In the current bounded live atomic canonical MBTT mode, the engine selects a Step-15 temporal-cohesive shell with $\kappa=8$, $\nu=103$, and $\rho=12.88$, clearing the step-15 bar (7.40) by an absolute overshoot of $5.48$, the largest in the sequence.
...
\begin{itemize}[nosep]
    ...
    \item The terminal DCT fixed-point signal (\cref{thm:spectral}): in the current bounded live atomic canonical MBTT lane, the temporal-cohesive shell is selected at step~15 with $\nu=103$, $\kappa=8$, and the largest absolute overshoot in the sequence.
\end{itemize}
...
More abstractly, PEN should be read as one executable articulation of a broader stage-local selective schema: admissibility fixes the viable field, the bar enforces context-sensitive sufficiency, semantic minimality enforces constructive primeness, and integration updates the realized library. Other implementations could refine the local comparative functional or tie-breaking discipline without changing that abstract architecture.
...
\begin{enumerate}[nosep, start=4]
    \item \textbf{Terminal temporal-cohesive fixed point.} In PEN's current bounded live atomic canonical MBTT lane, the temporal-cohesive Step-15 AST is selected with $\nu=103$, $\kappa=8$, and maximal absolute overshoot; its semantic completion is read as DCT, and Step~16 then halts by the internal/external combinatorial squeeze.
    This identifies a terminal fixed point of PEN only within that disclosed lane and connected component, not as grammar-independent empirical evidence across hostile grammars.
...
\emph{Conjecture:} with sufficiently powerful search over the same disclosed canonical MBTT space, the same structural PEN trajectory, including the same temporal-cohesive endpoint and the same semantic identifications, would emerge without the current tractability guidance.
