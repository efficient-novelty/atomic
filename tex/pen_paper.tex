...
In the current strict bounded recovery lane, this shell is evaluated at conceptual cost $\kappa=8$ and yields the canonical executable value $\nu = 103$, hence $\rho = 12.88$.
...
The canonical Step-15 count is now the machine value produced by the current executable evaluator:
\[
  \nu_{\mathrm{exec}}(\mathrm{DCT}) = 103.
\]
...
The current paper value is the executable result that actually drives selection in the artifact.
...
The DCT's canonical Generative Capacity is the executable value
\[
  \nu(\mathrm{DCT}) = 103.
\]
That value is produced by the current evaluator from the conceptual temporal shell together with its structural temporal-cohesive interaction payload, and it is the theorem-level DCT novelty value used in this manuscript.
...
\begin{enumerate}[nosep]
    ...
    $\nu(R_{15}) = 103$ is the canonical machine value in the current strict bounded recovery lane.
...
canonical quantitative claim is the executable structural total $\nu=103$;
