...
\subsection{\texorpdfstring{The $\rho = 12.88$ Efficiency Peak}{The rho = 12.88 Efficiency Peak}}
\label{sec:rho-spike}

The executable efficiency ratio at Step~15 is $\rho = 103/8 = 12.88$, clearing the bar ($7.40$) by an absolute overshoot of $5.48$---the largest in the entire 15-step sequence~\cite{pen}.
...
The manuscript now treats the executable structural total $\nu = 103$ as canonical~\cite{pen}.
...
\subsection{\texorpdfstring{The $\nu = 103$ Explosion and the Inflationary Epoch}{The nu = 103 Explosion and the Inflationary Epoch}}
...
In the current executable lane, this temporal-cohesive shell is evaluated at $\kappa = 8$ and yields the canonical value $\nu = 103$.
...
The combinatorial amplification at Step~15 ($\nu = 103$, compared to $\nu \leq 62$ at all prior steps) is a necessary structural precondition for any later physical inflationary dynamics.
...
The physical state at the onset of dynamics ($t = 0$) is specified by the complete DCT library $\B_{15}$, whose total specification complexity is $\kappa_{\mathrm{total}} = \sum_{i=1}^{15} \kappa_i = 64$ clauses (equivalently, 610~bits in MBTT encoding~\cite{pen}).
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Structural inflation} is the combinatorial schema explosion ($\nu = 103$) produced by the sudden cross-interaction of 14~previously disconnected kinematic structures; it supplies the precondition for, rather than a replacement of, any later metric inflation.
