...
\begin{itemize}[nosep]
  ...
  \item a legacy paper-calibrated replay with the same 15-step order, retained only as provenance rather than as the current quantitative canon;
...
\paragraph{Late-step canonicalization.}
The current repository documentation uses a single authoritative late-step novelty table: $(46,62,103)$ for Metric, Hilbert, and DCT in the executable canon. Older paper-calibrated and pre-canonical notes are retained only as provenance and should not be cited as current quantitative claims.
