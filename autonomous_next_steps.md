...
`reference` refinements too, above their narrower active clause-`5`
`claim_flat_codomain` clause-`3`
...
looser recombined active-window plus self-contained family too, and above the
full historical-reanchor plus active-window plus self-contained qualification
triad too while now targeting a different code-side step-`15` repair class
...
bucket. The first looser recombined active-window plus self-contained family
on those same active clause-`5` buckets also qualifies the targeted
claim-side parent shell, yet both broad looser recombined surfaces reland
that same unsafe matched control with the same noncanonical `60 / 8`, the
same `4427 / 545 / 2247`, `incumbent_dominance = 110`,
`small_cluster = 2952 / 558 / 558 / 108`, and the same reopened `single`
bucket. Granting that same parent shell the full historical-reanchor plus
active-window plus self-contained triad still relands that same unsafe
matched control with the same noncanonical `60 / 8`, the same
`4427 / 545 / 2247`, `incumbent_dominance = 110`,
`small_cluster = 2952 / 558 / 558 / 108`, and the same reopened `single`
bucket. The parent-route, alternate active-window / self-contained, looser
recombined active-window plus self-contained, and full qualification-triad
families are therefore frozen as an unsafe negative-control lattice, and
their narrower alternate clause-`6` refinements are frozen as a split unsafe
control rather than the landed repair. Narrowing those same alternate
active-window and
self-contained families one layer earlier to only one claim-flat clause-`3`
argument branch on the active clause-`5` `claim_flat_codomain` bucket also
stays unsafe rather than exposing a repair: on either
`claim_flat_argument` or `claim_eventual_argument`, both alternate families
reland noncanonical `60 / 8` with `retained = 2`,
`generated = 4379`, `partial_prefix_bar_failure = 549`,
`incumbent_dominance = 110`, zero-admitted captures `2259`, first-mismatch
`308 / 177 / 50 / 14`, `small_cluster = 2880 / 486 / 486 / 108`, and the
same reopened `single` bucket. On that chosen active clause-`5` bucket, both
alternate clause-`3` refinements keep the same four targeted remaining-two
parent cells but now only remove `4` zero-admitted captures and `12`
remaining-one pruned prefixes in total. Keep the demoted claim-safe mismatch-`1`
...
claim-flat clause-`3` refinements inside that same parent-route family, plus
...
Most recently on the exact-screen engine path, that broader remaining-two
follow-on was split one layer deeper across its clause-`5` cells too. The
whole mismatch-`0` clause-`4` `claim_next_bridge` release still lands the
broader `4763 / 517 / 2271` tradeoff with widened
`small_cluster = 3456 / 522 / 522 / 0`, but the three narrower clause-`5`
cells all reland the same smaller control:
- `claim_flat_codomain = 4475 / 541 / 2271`
- `claim_next_codomain = 4475 / 541 / 2271`
- `reference = 4475 / 541 / 2271`

All three keep accepted step `15` canonical `103 / 8`, keep
`incumbent_dominance = 3`, keep the fenced `single` pocket, widen
`small_cluster` only to `3240 / 522 / 522 / 0`, contract every live
mismatch-`0` pair from `42` to `40`, contract the clause-`4` split only from
...
- Any remaining step-`15` leverage is therefore no longer on that broad
  ...
  mass.
- That broader exact-screen release is now also spent one layer deeper across
  its clause-`5` cells:
  - the `claim_flat_codomain`, `claim_next_codomain`, and `reference` cells
    each reland the same smaller `4475 / 541 / 2271` tradeoff
  ...
  - each probe localizes to only its chosen `claim_next_bridge`
    clause-`5` cell while leaving the two sibling `claim_next_bridge` cells
    at `48` and the whole `reference` half at `36`
- Any remaining step-`15` leverage is therefore no longer on clause-`5`
...
- Narrowing those same alternate active-window and self-contained families one
  ...
  - on either `claim_flat_argument` or `claim_eventual_argument`, both
    ...
    `generated = 4379`, `partial_prefix_bar_failure = 549`,
    `incumbent_dominance = 110`, zero-admitted captures `2259`, the same
    `308 / 177 / 50 / 14` first-mismatch distribution, the same
    `small_cluster = 2880 / 486 / 486 / 108`, and the same reopened
    `single` bucket
  - on the chosen active clause-`5` bucket, both alternate clause-`3`
    ...
    parent cells, but now only `1` removed capture and `3` removed pruned
    prefixes per cell, for a narrowed targeted delta of `4` removed captures
    plus `12` removed pruned prefixes with no off-target family introduced
- Any remaining step-`15` leverage is therefore no longer on the narrower
  alternate active-window or self-contained claim-flat clause-`3`
  refinements either. Even this smaller `4379 / 549 / 2259` split control
  still displaces the canonical `103 / 8` winner and reopens the `single`
  bucket, so the next honest probe has to stay above those alternate
  clause-`3` refinements as well as above their broader and clause-`6`
  siblings.
...
- The sibling recombined parent-level qualification family above that same
  ...
  - on either active clause-`5` family, that hybrid relands the exact same
    ...
    representative clause-`4` split `20 / 14`, and the same chosen active
    clause-`4` plus clause-`5` bucket at `44`
- Any remaining step-`15` leverage is therefore no longer on that sibling
  recombined historical-reanchor plus self-contained hybrid either. Adding
  historical reanchor on top of the already-spent self-contained family does
  not create a fresh parent-level repair class, so the next honest probe has
...
- Any remaining step-`15` leverage is therefore no longer on that first
  ...
  it rather than reopening narrower clause-`3` / clause-`6` selectors.
...
- That full qualification triad is also localized at the same targeted delta
  ...
  - it removes the same `8` zero-admitted captures and the same `24`
    remaining-one pruned prefixes
  - it introduces no off-target exact-prune or pruned-prefix family
- Any remaining step-`15` leverage is therefore no longer on that next
  looser recombined full qualification triad either. Exhausting
  historical-reanchor, active-window, and self-contained together does not
...
- The sibling clause-`4` `reference` splits on those same alternate
  active-window and self-contained families are now also spent across both
  active clause-`5` buckets:
  - a scoped clause-`4` `reference` override on either alternate family does
    ...
    `claim_next_bridge` branch, the sibling reference clause-`2` sheet, and
    lifted terminals fenced
  - on `claim_flat_codomain` and `reference`, both alternate clause-`4`
    splits reland the same smaller unsafe matched control with noncanonical
    `60 / 8`, `retained = 2`, `generated = 4391`,
    `partial_prefix_bar_failure = 557`, `incumbent_dominance = 110`,
    zero-admitted captures `2271`, first-mismatch distribution
    `312 / 177 / 50 / 14`, and `small_cluster = 2880 / 486 / 486 / 108`
  - the isolated `single` bucket reopens to `2` fully scored non-winners plus
    `2` residual prunes at best overshoot `545 / 5278`
  ...
  - each contracts only the chosen clause-`4` `reference` plus active
    clause-`5` bucket to `36`
- Those alternate clause-`4` splits are also already localized at delta
  level:
...
- Any remaining step-`15` leverage is therefore no longer on those alternate
  active-window or self-contained clause-`4` `reference` splits either.
  They stay matched across both alternate families and both active
...
4. Spend the next execution turn on the next fresh code-side step-`15`
   ...
   - keep the first recombined representative mismatch-`0` claim-side
     ...
     `single` bucket
   - keep the sibling recombined representative mismatch-`0` claim-side
     historical-reanchor plus self-contained family frozen too; on both active
     clause-`5` `claim_flat_codomain / reference` buckets it relands that same
     unsafe `4427 / 545 / 2247` self-contained shell with noncanonical
   ...
   - with the first clause-`1` sibling exact-screen pair on
     `claim_flat_domain / claim_sharp_codomain / claim_next_bridge / claim_next_codomain`
     and its same-boundary `claim_flat_domain` reland now spent too, keep
     that pair-level clause-`2` lattice and same-layer exact-screen boundary
     frozen as controls rather than fresh leverage
   - the released first clause-`1` sibling exact-screen
     `claim_flat_domain` plus clause-`5` `claim_next_codomain` parent is now
     also spent on remaining-one exact-summary relief: it stays on the same
     `4343 / 552 / 2271`, keeps first-mismatch counts `311 / 177 / 50 / 14`,
     keeps `small_cluster generated = 3141`, keeps exact-prune family
...
- another pair-cell remaining-one exact-summary relief pass on any of the
  ...
  clause-`5` families
- another remaining-one exact-summary relief pass beneath the released first
  clause-`1` sibling exact-screen `claim_flat_domain` plus
  `claim_next_codomain` parent on
  `claim_flat_domain / claim_sharp_codomain / claim_flat_domain /
  claim_next_bridge / claim_next_codomain`; it now stays neutral on the same
  `4343 / 552 / 2271`, the same `311 / 177 / 50 / 14`, the same
  `small_cluster = 3141 / 522 / 522 / 0`, the same exact-prune family
  `((0, None, None), 2271)`, the same `6813` structurally connected but
  unqualified generated candidates, and no released remaining-one
  pruned-terminal group
...
- another representative mismatch-`0` claim-side active-window or
  self-contained clause-`4` `reference` split on
  `claim_eventual_domain / claim_next_codomain / reference /
  claim_flat_codomain`; those narrower alternate qualification families now
  only reland the same smaller unsafe `4391 / 557 / 2271` shell with
...
- another representative mismatch-`0` claim-side active-window or
  self-contained clause-`4` `reference` split on
  `claim_eventual_domain / claim_next_codomain / reference / reference`;
  those sibling narrower alternate qualification families now reland the
  same smaller unsafe `4391 / 557 / 2271` shell with the same noncanonical
  `60 / 8`, the same `incumbent_dominance = 110`, the same
  `small_cluster = 2880 / 486 / 486 / 108`, the same reopened `single`
  bucket, the same `15 / 15 / 12` clause-`2` spread, the same
  `24 / 18` clause-`4` split, and the same two-remaining-three-to-four-
...
- another representative mismatch-`0` claim-side active-window or
  self-contained reland narrowed to only `claim_flat_argument` or only
  `claim_eventual_argument` on
  `claim_eventual_domain / claim_next_codomain / claim_next_bridge /
  claim_flat_codomain`; those narrower alternate clause-`3` refinements now
  only reland unsafe noncanonical `60 / 8` with `retained = 2`,
  `generated = 4379`, `partial_prefix_bar_failure = 549`,
  `incumbent_dominance = 110`, zero-admitted captures `2259`,
  `small_cluster = 2880 / 486 / 486 / 108`, the same reopened `single`
  bucket, and the same targeted four-cell plus `12`-pruned-prefix delta on
  the active `claim_flat_codomain` bucket
...
- the sibling clause-`4` `reference` splits on those same alternate
  active-window and self-contained families are also pinned across both
  active clause-`5` `claim_flat_codomain / reference` buckets as a matched
  smaller unsafe split control rather than fresh repairs:
  noncanonical `60 / 8`, `retained = 2`, `generated = 4391`,
  `partial_prefix_bar_failure = 557`, `incumbent_dominance = 110`,
  zero-admitted captures `2271`,
  `small_cluster = 2880 / 486 / 486 / 108`, the same reopened `single`
  bucket, the same `15 / 15 / 12` clause-`2` spread, and the same
  `24 / 18` clause-`4` split
- those alternate clause-`4` splits are also localized to only two removed
  remaining-three clause-`4` `reference` parent captures, four introduced
...
- the narrower clause-`3` `claim_flat_argument / claim_eventual_argument`
  refinements of those same alternate active-window and self-contained
  families on the active clause-`5` `claim_flat_codomain` bucket are also
  pinned as a matched smaller unsafe split control rather than fresh repairs:
  both branches reland noncanonical `60 / 8` with `retained = 2`,
  `generated = 4379`, `partial_prefix_bar_failure = 549`,
  `incumbent_dominance = 110`, zero-admitted captures `2259`,
  `small_cluster = 2880 / 486 / 486 / 108`, and the same reopened
  `single` bucket
- those narrower alternate-family clause-`3` refinements also keep the same
  `308 / 177 / 50 / 14` first-mismatch distribution while contracting the
  targeted delta to the same four remaining-two parent cells but only `4`
  removed captures plus `12` removed remaining-one pruned prefixes in total
...
- the sibling recombined representative mismatch-`0` claim-side
  historical-reanchor plus self-contained family is also pinned across both
  active clause-`5` `claim_flat_codomain / reference` buckets as the exact
  same unsafe shell as the already-spent self-contained family:
  noncanonical `60 / 8`, `retained = 2`, `generated = 4427`,
  `partial_prefix_bar_failure = 545`, `incumbent_dominance = 110`,
  zero-admitted captures `2247`,
  `small_cluster = 2952 / 558 / 558 / 108`, the same reopened `single`
...
- the first looser recombined representative mismatch-`0` claim-side
  ...
  `20 / 14` clause-`4` split, and the same chosen active clause-`4` plus
  clause-`5` bucket at `44`
- the next looser recombined representative mismatch-`0` claim-side
  historical-reanchor plus active-window plus self-contained qualification
  triad is also pinned across both active clause-`5`
  `claim_flat_codomain / reference` buckets as that same exact unsafe shell:
  noncanonical `60 / 8`, `retained = 2`, `generated = 4427`,
  `partial_prefix_bar_failure = 545`, `incumbent_dominance = 110`,
  zero-admitted captures `2247`,
  `small_cluster = 2952 / 558 / 558 / 108`, the same reopened `single`
...
- the three narrower clause-`5` exact-screen splits below that broad release
  are also pinned as symmetric smaller tradeoff controls rather than fresh
  repairs:
  `claim_flat_codomain = 4475 / 541 / 2271`,
  `claim_next_codomain = 4475 / 541 / 2271`, and
  `reference = 4475 / 541 / 2271`, all with accepted step `15` canonical
  `103 / 8`, `incumbent_dominance = 3`, a fenced `single` pocket, and
  widened `small_cluster = 3240 / 522 / 522 / 0`
- those narrower clause-`5` exact-screen splits are also localized as
  single-cell contractions rather than distinct repair classes:
  every live mismatch-`0` clause-`0` / clause-`1` pair contracts only to
  `40`, the clause-`4` split contracts only to `22 / 18`, zero-admitted
...
- the first clause-`1` sibling exact-screen `claim_flat_domain` sheet on that
  ...
  scope at the exact-screen boundary, and introduces no new exact-prune or
  pruned-terminal family
- the remaining-one exact-summary follow-on beneath that released first
  clause-`1` sibling exact-screen `claim_flat_domain` plus clause-`5`
  `claim_next_codomain` parent is also pinned as a neutral control rather
  than fresh leverage:
  it keeps the same `4343 / 552 / 2271`, the same
  `311 / 177 / 50 / 14`, the same `small_cluster generated = 3141`, the same
  exact-prune family `((0, None, None), 2271)`, the same `6813`
  structurally connected but unqualified generated candidates, and releases
  no remaining-one pruned-terminal group
