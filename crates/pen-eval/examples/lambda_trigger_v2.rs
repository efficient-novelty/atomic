use pen_eval::lambda_trigger_v2::build_lambda_trigger_v2_report;
use std::env;
use std::fs;
...
fn main() {
    ...
    let report = build_lambda_trigger_v2_report(freeze_commit, eval_commit);
    let json = serde_json::to_string_pretty(&report).expect("serialize v2 report");
