...
#[cfg(test)]
fn override_claim_step_fifteen_clause_one_flat_codomain_on_clause_zero_claim_flat_side_pocket()
...
#[cfg(test)]
fn override_claim_step_fifteen_clause_five_side_pocket_on_claim_safe_clause_zero_one()
...
#[cfg(test)]
fn override_claim_step_fifteen_clause_five_remaining_two_mismatch_zero_bridge_slice()
    ...
    fn current_claim_step_fifteen_anchor_eleven_exact_argument_pocket_keeps_claim_variants_non_winning_on_bit_cost()
     {
        ...
        assert_eq!(
            ...
            .into_iter()
            .collect(),
            "the anchor-11 exact-argument pocket should stay frozen as a structurally tie-clean 103/8 rival that differs from the canonical step-15 winner only by higher bit cost, even after the live claim variants consume it"
        );
    }
    ...
    fn current_claim_step_fifteen_claim_clause_two_prefixes_expose_only_anchor_eleven_exact_argument_pocket()
     {
        ...
        for clause_two in claim_clause_two_variants.iter().cloned() {
            ...
            assert_eq!(
                observed_exprs.len(),
                5,
                "the exact anchor-11 pocket should now also inject the two clause-5 side options even before the clause-4 side pocket is present"
            );
            assert!(
                observed_exprs
                    .contains("Pi(Sharp(Eventually(Sharp(Var(1)))), Eventually(Sharp(Var(1))))")
            ...
            for clause_one in clause_one_variants.iter().cloned() {
                for clause_two in clause_two_variants.iter().cloned() {
                    ...
                    assert!(
                        ...
                        "the exact remaining-two mismatch-0 bridge slice should stay legal enough to reach the clause-5 injector"
                    );
                    let work_item = create_online_prefix_work_item(
                        8,
                        prefix,
    ...
    fn current_claim_step_fifteen_anchor_eleven_exact_argument_pocket_stays_tie_clean_until_clause_six_moves()
     {
        ...
        for (clause_two_label, clause_two) in clause_two_variants {
            for mask in 0u8..(1 << side_positions.len()) {
                ...
                assert!(
                    admissibility_decision.is_admitted(),
                    "the anchor-11 exact-argument pocket should stay locally admissible across the repaired-side subsets being diagnosed"
                );
                let exact_nu = u16::try_from(
                    structural_nu(&telescope, &surface.library, &surface.nu_history).total,
                )
        ...
        assert_eq!(
            ...
            .into_iter()
            .collect(),
            "the anchor-11 exact-argument pocket should stay non-winning across every repaired-side subset of positions 0, 1, 4, and 5, while clause 6 still remains the boundary where the mixed claim clause-2 pocket reopens the unsafe 89/8 rival"
        );
    }
    ...
    fn current_claim_step_fifteen_clause_five_remaining_two_mismatch_zero_bridge_slice_stays_a_negative_control()
     {
        ...
        assert_eq!(step_fifteen.telescope, Telescope::reference(15));
        assert_eq!(step_fifteen.demo_funnel.generated_raw_prefixes, 4691);
        assert_eq!(
            step_fifteen.exact_screen_reasons.partial_prefix_bar_failure, 589,
            "opening the clause-5 side pocket only on the exact remaining-two mismatch-0 bridge slice should still stay a widening negative control because it lifts breadth while widening the clean partial-prefix wall"
        );
        assert_eq!(
            step_fifteen.exact_screen_reasons.incumbent_dominance, 3,
            "the exact remaining-two mismatch-0 bridge-slice reland should keep the residual single-bucket incumbent fence unchanged"
        );
        assert_eq!(wall_summary.capture_count, 589);
        assert_eq!(
            bucket_stats
                .get("k8:structural_generic:temporal_operator:library_backed:small_cluster"),
            Some(&DemoBucketStats {
            ...
            "the exact remaining-two mismatch-0 bridge-slice reland should widen the same small-cluster bucket rather than touching the isolated single pocket"
        );
        assert_eq!(
            bucket_stats.get("k8:structural_generic:temporal_operator:library_backed:single"),
            Some(&DemoBucketStats {
    ...
    fn current_claim_step_fifteen_clause_five_side_pocket_on_claim_safe_clause_zero_one_surface_stays_a_negative_control()
     {
        ...
        assert_eq!(step_fifteen.telescope, Telescope::reference(15));
        assert_eq!(step_fifteen.demo_funnel.generated_raw_prefixes, 4779);
        assert_eq!(
            step_fifteen.exact_screen_reasons.partial_prefix_bar_failure, 585,
            "broadening the clause-5 side pocket onto the claim-safe clause-0/1 surface should stay a widening negative control by reopening more partial-prefix wall pressure"
        );
        assert_eq!(
            step_fifteen.exact_screen_reasons.incumbent_dominance, 3,
            "the widened clause-5 side-pocket reland should keep the residual single-bucket incumbent fence unchanged"
        );
        ...
        assert_eq!(
            bucket_stats
                .get("k8:structural_generic:temporal_operator:library_backed:small_cluster"),
            Some(&DemoBucketStats {
                generated_terminal_candidates: 3516,
                admissible_terminal_candidates: 586,
                exact_screened_terminal_candidates: 586,
                pruned_terminal_candidates: 0,
                fully_scored_terminal_candidates: 0,
                best_overshoot: None,
            }),
            "the clause-5 side-pocket reland on the broader claim-safe clause-0/1 surface should widen the same small-cluster bucket rather than touching the isolated single pocket"
        );
        assert_eq!(
            bucket_stats.get("k8:structural_generic:temporal_operator:library_backed:single"),
            Some(&DemoBucketStats {
            ...
            "the widened clause-5 side-pocket reland should keep the isolated single pocket fenced even while it widens the noncanonical small-cluster surface"
        );
    }
    ...
    fn current_claim_step_fifteen_exact_family_same_primary_relief_still_unfences_the_isolated_single_pocket()
     {
        ...
        assert_eq!(
            step_fifteen.exact_screen_reasons.partial_prefix_bar_failure, 553,
            "family-local same-primary relief should now still collapse the remaining incumbent pressure only by widening the repaired local surface to 4331 and keeping the partial-prefix wall at 553"
        );
        assert_eq!(
    ...
    fn step_thirteen_positions_zero_four_five_six_negative_control_reopens_local_floor_but_still_breaks_step_thirteen_parity()
     {
        ...
        assert_eq!(
            accepted_late_path
                ...
                .collect::<Vec<_>>(),
            vec![(14, 62, 9, 12027), (15, 103, 8, 4331)],
