...
//!   digests.
//!
//! This module freezes the capability shapes, reports the bridge
//! obligation ledger, and (as of Phase G) provides the three deterministic
//! bridge constructors: the independent Rust replay through the unchanged
//! kernel and synthesis checker, the safe-Agda acceptance of the generated
//! input package under the pinned checker, and the exact common-input and
//! common-transcript byte agreement. Each constructor is a deterministic
//! replay/check gate; there is no deserialization path and no digest-only
//! escape hatch, and this module cannot mint any of the four
//! correspondence capabilities in `production_refinement_theorem` — as of
//! Phase H those are minted only by the single private factory
//! (`production_refinement_theorem::correspondence_factory`), which
//! consumes the four capabilities this module's constructors produce.
...
/// The one exact accepted-section mask a genuine acceptance run emits.
/// Crate-private so the acceptance constructor and the correspondence
/// factory derive the identical value; callers still cannot supply one.
pub(crate) fn expected_accepted_production_section_mask_v1() -> AcceptedProductionSectionMaskV1 {
    let mut mask = AcceptedProductionSectionMaskV1 {
        sections: Arc::<[AcceptedProductionSectionV1]>::from(
            REQUIRED_ACCEPTED_PRODUCTION_SECTIONS_V1,
        ),
        digest: Digest::of_bytes(b"pending accepted production section mask"),
    };
    mask.digest = Digest::of_canonical(
...
/// Assemble the exact generated safe-Agda acceptance package for one
/// canonical bundle byte vector and one Rust-rendered transcript byte
/// vector: the fixed acceptance entry, the compile-time-fixed wire
/// source tree, and the two generated byte-list modules, in the pinned
/// checker's exact reporting order. The generated input module is
/// round-trip verified against the supplied bundle bytes before it
/// enters the package. Shared by the Phase G acceptance constructor
/// (which feeds the package to the pinned checker) and the Phase H
/// correspondence factory (which re-derives the package identity a
/// genuine acceptance run must carry from bytes it has already
/// byte-compared).
pub(crate) fn assemble_wire_acceptance_package_v1(
    canonical_bytes: &[u8],
    rust_transcript: &[u8],
) -> Result<
    (Vec<PackageSourceV1>, VerifiedAgdaProductionInputArtifactV1),
    ProductionBridgeFailureV1,
> {
    let input_source = render_production_bundle_input_module_v1(canonical_bytes)?;
    let input_artifact =
        verify_agda_production_input_artifact_v1(canonical_bytes, input_source.as_bytes())?;
    let transcript_source = render_production_transcript_input_module_v1(rust_transcript)?;
    ...
    for source in WIRE_ACCEPTANCE_STATIC_SOURCES_V1 {
        sources.push(PackageSourceV1::Fixed(*source));
    }
    sources.push(PackageSourceV1::Generated(GeneratedAgdaSourceV1 {
...
pub fn verify_agda_production_acceptance_v1(
    ...
    let (sources, input_artifact) =
        assemble_wire_acceptance_package_v1(bundle.canonical_bytes(), replay.rust_transcript())?;
    let package = diagnose_generated_agda_package_v1(
        &sources,
        GENERATED_ACCEPTANCE_ENTRY_RELATIVE_PATH_V1,
    )?;

    let accepted_sections = expected_accepted_production_section_mask_v1();
...
/// Discharged as of Phase H: the single private correspondence factory
/// (`production_refinement_theorem::correspondence_factory`) is
/// implemented, so no bridge obligation lacks an implementation. The
/// list is retained (empty) because the frontier type and the
/// manifest-only diagnostic below remain the fail-closed path for
/// callers that do not present the four bridge capabilities.
...
/// Diagnose the V2 canonical production bridge from a manifest alone.
///
/// This function first binds the exact unchanged V3 candidate and then
/// fails closed: a manifest by itself can never mint bridge authority.
/// As of Phase H every bridge obligation has an implementation (the
/// unresolved list in the reported frontier is empty), but authority is
/// minted only by the single private correspondence factory consuming
/// the four actual capabilities. This diagnostic cannot be made to
/// succeed by supplying bundle or transcript digests: it accepts
/// neither.
...
mod tests {
    ...
    #[test]
    fn exact_v3_still_fails_closed_from_a_manifest_alone() {
        ...
        let CanonicalProductionAuthorityFailureV1::MissingCanonicalBridgeAuthorities { frontier } =
            diagnose_canonical_production_authority_v1(&manifest)
                .expect_err("a manifest alone must never mint bridge authority")
        else {
            panic!("exact V3 must stop at the canonical bridge frontier");
        };
        // Phase H: every bridge obligation has an implementation, so the
        // unresolved-implementation list is empty; the diagnostic still
        // fails closed because no capability was presented.
        assert_eq!(
            frontier.unresolved_obligations(),
            CANONICAL_PRODUCTION_AUTHORITY_FRONTIER_V1
        );
        assert!(frontier.unresolved_obligations().is_empty());
        assert_eq!(
            COMPLETED_CANONICAL_PRODUCTION_PREREQUISITES_V1.last(),
            Some(&CanonicalProductionAuthorityObligationV1::SinglePrivateCorrespondenceFactory)
        );
        assert_eq!(
