...
- A newer local step-`15` survivor-bucket regression now freezes the latest
  ...
  - the remaining exact-screened pressure no longer dies inside the
    `small_cluster` bucket:
...
- A newer local step-`15` small-cluster relief now lands on top of that
  ...
  - local step-`15` incumbent-dominance prunes now fall from `246` to `3`
    ...
    stay `472`
  - the remaining incumbent pressure no longer sits on the same-primary
    `small_cluster` surface; it now survives only as `3` residual
...
- A newer local step-`15` clause-`4` demo-sharp-bridge side-pocket repair
  ...
  - the remaining local pressure is now `476` partial-prefix bar failures
...
- The earlier raw isolated late-side reland probes are now split more cleanly:
  ...
  - the clause-`4` / clause-`5` side pocket still stays tiny and
    ...
    residual `3` single-bucket incumbent prunes on top of the then-current `4056`
...
- The current local step-`15` diagnostic suite was rerun green again after
  landing the narrow step-`15` small-cluster relief on top of the fenced
  clause-`5` side-pocket repair:
  ...
  - the new
    ...
    regression is green
  - the new
    `connectivity_accepts_clause_five_demo_sharp_domain_only_on_the_exact_anchor_eleven_clause_four_side_pocket`
    regression is green
  - `stored_claim_v11_certificate_and_step_15_live_checkpoint_freeze_current_canonical_diagnosis`
    is green
  - `stored_claim_v11_benchmark_freezes_runtime_and_floor_counts` is green
- The executable late-path guardrails are now synced to that same latest
  clause-`4` / clause-`5` side-pocket-plus-small-cluster local surface too:
  ...
  - the step-`15` late-path guardrails now freeze the current
    `4088` / `3` / `3012 / 502 / 502 / 0` local surface rather than the
  ...
  - the `[5,1,3,3,5,3,3]` step-`13` negative control still preserves guarded
    ...
    local step-`15` generated surface at `4088`
...
- Keep the current local step-`13` / step-`14` / step-`15` surfaces green as
  ...
  local post-`v11` step `15` guardrail should stay `DCT 103 / 8 / 4088`
  with `476` partial-prefix bar failures, `3` incumbent-dominance prunes, and
  a `3012 / 502 / 502 / 0` `small_cluster` surface plus the fenced
...
- Treat stored `v11` step `15 = 3972 / 5000` as the remaining stored
  late-floor miss, but keep the new local `4088` guardrail explicit too; do
...
3. Use the stored `v11` certificate and late-step live checkpoints as the
   ...
   late chain at `DCT 103 / 8 / 4088`, with `476` partial-prefix bar
   failures, only `3` incumbent-dominance prunes, a widened temporal
   `small_cluster` at `3012 / 502 / 502 / 0`, the isolated `single` pocket
   ...
   exact three families keeps `4088` / `476`, collapses the remaining
